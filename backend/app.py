...
def build_feature_vector(data: dict) -> list:
    """Build a feature vector aligned to config['feature_columns'] (39 features)."""
    # Scalar inputs from request
    scalar_map = {
        'age':                       float(data.get('age', 30)),
        'account_age_months':        float(data.get('account_age_months', 12)),
        'monthly_fee':               float(data.get('monthly_fee', 12.99)),
        'devices_used':              float(data.get('devices_used', 2)),
        'avg_watch_time_minutes':    float(data.get('avg_watch_time_minutes', 120)),
        'watch_sessions_per_week':   float(data.get('watch_sessions_per_week', 10)),
        'binge_watch_sessions':      float(data.get('binge_watch_sessions', 5)),
        'completion_rate':           float(data.get('completion_rate', 50)),
        'rating_given':              float(data.get('rating_given', 3.0)),
        'content_interactions':      float(data.get('content_interactions', 15)),
        'recommendation_click_rate': float(data.get('recommendation_click_rate', 5)),
        'days_since_last_login':     float(data.get('days_since_last_login', 5)),
    }

    # Derived engineered features
    avg_watch = scalar_map['avg_watch_time_minutes']
    devices    = max(scalar_map['devices_used'], 1)
    sessions   = max(scalar_map['watch_sessions_per_week'], 1)
    binge      = scalar_map['binge_watch_sessions']
    fee        = max(scalar_map['monthly_fee'], 0.01)
    scalar_map['watch_time_per_device']  = avg_watch / devices
    scalar_map['engagement_ratio']       = binge / sessions
    scalar_map['value_ratio']            = avg_watch / fee
    scalar_map['binge_watch_frequency']  = binge / max(sessions, 1)

    # One-hot fields derived from string request params
    gender    = data.get('gender', 'Female')
    country   = data.get('country', 'USA')
    sub_type  = data.get('subscription_type', 'Standard')
    payment   = data.get('payment_method', 'Credit Card')
    device    = data.get('primary_device', 'Laptop')
    fav_genre = data.get('favorite_genre', 'Drama')

    one_hot_map = {
        'gender_Male':                   1.0 if gender == 'Male' else 0.0,
        'gender_Other':                  1.0 if gender == 'Other' else 0.0,
        'country_Brazil':                1.0 if country == 'Brazil' else 0.0,
        'country_Canada':                1.0 if country == 'Canada' else 0.0,
        'country_France':                1.0 if country == 'France' else 0.0,
        'country_Germany':               1.0 if country == 'Germany' else 0.0,
        'country_India':                 1.0 if country == 'India' else 0.0,
        'country_Japan':                 1.0 if country == 'Japan' else 0.0,
        'country_Spain':                 1.0 if country == 'Spain' else 0.0,
        'country_UK':                    1.0 if country == 'UK' else 0.0,
        'country_USA':                   1.0 if country == 'USA' else 0.0,
        'subscription_type_Premium':     1.0 if sub_type == 'Premium' else 0.0,
        'subscription_type_Standard':    1.0 if sub_type == 'Standard' else 0.0,
        'payment_method_Debit Card':     1.0 if payment == 'Debit Card' else 0.0,
        'payment_method_PayPal':         1.0 if payment == 'PayPal' else 0.0,
        'payment_method_UPI':            1.0 if payment == 'UPI' else 0.0,
        'primary_device_Mobile':         1.0 if device == 'Mobile' else 0.0,
        'primary_device_Smart TV':       1.0 if device == 'Smart TV' else 0.0,
        'primary_device_Tablet':         1.0 if device == 'Tablet' else 0.0,
        'favorite_genre_Comedy':         1.0 if fav_genre == 'Comedy' else 0.0,
        'favorite_genre_Documentary':    1.0 if fav_genre == 'Documentary' else 0.0,
        'favorite_genre_Drama':          1.0 if fav_genre == 'Drama' else 0.0,
        'favorite_genre_Horror':         1.0 if fav_genre == 'Horror' else 0.0,
        'favorite_genre_Romance':        1.0 if fav_genre == 'Romance' else 0.0,
        'favorite_genre_Sci-Fi':         1.0 if fav_genre == 'Sci-Fi' else 0.0,
        'favorite_genre_Thriller':       1.0 if fav_genre == 'Thriller' else 0.0,
    }

    # Use config feature_columns order if available, else fall back to known order
    feature_columns = (
        config.get('feature_columns') if config and isinstance(config, dict) else None
    ) or [
        ...
        'country_India', 'country_Japan', 'country_Spain', 'country_UK', 'country_USA',
        'subscription_type_Premium', 'subscription_type_Standard',
        'payment_method_Debit Card', 'payment_method_PayPal', 'payment_method_UPI',
        'primary_device_Mobile', 'primary_device_Smart TV', 'primary_device_Tablet',
        ...
        'favorite_genre_Thriller',
        'watch_time_per_device', 'engagement_ratio', 'value_ratio', 'binge_watch_frequency',
    ]

    all_vals = {**scalar_map, **one_hot_map}
    return [all_vals.get(col, 0.0) for col in feature_columns]
...
def get_personalized_recommendations():
    ...
    try:
        ...
        # Build correctly-sized feature vector using config['feature_columns']
        user_features = build_feature_vector(data)
        age = float(data.get('age', 30))
